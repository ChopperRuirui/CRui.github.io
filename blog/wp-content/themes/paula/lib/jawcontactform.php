...
/**
 * Description of jawcontactform
 *
 * @author Petr
 */
class jawcontactform {
       ...
       // var_dump($options); 
        ...
        if ($options && is_array($options)){
            foreach($options as $key => $option){
               if(isset($this->{$key} )) {$this->{$key}=$option;}           
            }           
        }
    }
      ...
      if ($_SESSION['jaw_use_captcha_'.$this->id]){ 
        ...
        <script>
            ...
            jQuery(document).ready(function(){
              ...
              jQuery('#id<?php echo $this->id; ?> .fc_mod_email,#id<?php echo $this->id; ?> .fc_mod_name').change(function(){jQuery('#id<?php echo $this->id; ?> .jawcaptcha').fadeIn('slow');});
			...
			<?php if ($_SESSION['jaw_use_captcha_'.$this->id]) { ?>
              ...
              function reloadCaptcha() {document.getElementById('captcha<?php echo $this->id; ?>').src = '<?php echo $src ?>?' + Math.random();}
                function thanks(){jQuery('<div class="form-done"><?php echo $this->thanks; ?></div>').prependTo('#id<?php echo $this->id; ?> .jaw_mod_contact');}
                jQuery('#id<?php echo $this->id; ?> form').submit(function() {
                    ...
                    function(data) {
                         if (data.error=='0') {
                            ...
                            if (data.type.e_name) jQuery('#id<?php echo $this->id; ?> .e_name').fadeIn(500);
                            if (data.type.e_mail) jQuery('#id<?php echo $this->id; ?> .e_mail').fadeIn(500);
                            if (data.type.e_text) jQuery('#id<?php echo $this->id; ?> .e_text').fadeIn(500);
                           <?php if ($_SESSION['jaw_use_captcha_'.$this->id]){ ?> 
                               if (data.type.e_captcha) jQuery('.e_captcha').fadeIn(500);
                               <?php } ?>
...
<?php if ($_SESSION['jaw_use_captcha_'.$this->id]){ ?>
                <div class="jawcaptcha">
                    ...
                    <div><img id="captcha<?php echo $this->id; ?>" class="captcha" src="<?php echo $src; ?>" alt="CAPTCHA Image" />
                        <a href="#" class="reload" style="float:right" onclick="document.getElementById('captcha<?php echo $this->id; ?>').src = '<?php echo $src ?>?' + Math.random(); return false" title="Different Image">
                            </a>
                        <div class="clear"></div>
                    </div>    
                    <input type="text" class="captcha_code" onfocus="if(this.value=='enter code') this.value='';" onblur="if(this.value=='') this.value='enter code';" value="enter code" name="captcha_code" size="10" maxlength="6" />
