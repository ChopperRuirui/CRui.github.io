...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="<?php echo $gantry->language; ?>" lang="<?php echo $gantry->language; ?>" <?php echo ($gantry->get('facebook-enabled')) ? 'xmlns:fb="http://www.facebook.com/2008/fbml"':''; ?> <?php echo ($gantry->get('googleplus-enabled')) ? 'itemscope itemtype="http://schema.org/"':''; ?>>
    <head>
        ...
        $gantry->displayHead();
        $gantry->addStyles(array('template.css', 'wordpress.css', 'wp.css', 'style.css','typography.css','prettyPhoto.css','slider_freshcubes.css'));
        wp_enqueue_script('jquery');
        $gantry->addScript(array('paula.js','jquery.grayscale.js'));
        $gantry->addScript('jquery.rotate.js');
        $gantry->addScript('jquery.preload.js');
        $gantry->addScript('jquery.prettyPhoto.js');
     
       
        /* LINK FACEBOOK */
        if ($gantry->get('post-facebook-enabled')){ 

		$gantry->addInlineScript('(function(d, s, id) {
			    var js, fjs = d.getElementsByTagName(s)[0];
			    if (d.getElementById(id)) return;
			    js = d.createElement(s); js.id = id;
			    js.src = "//connect.facebook.net/' . $gantry->get('post-facebook-locate')  . '/all.js#xfbml=1";
			    fjs.parentNode.insertBefore(js, fjs);
			  }(document, "script", "facebook-jssdk"));'
		      );

		//$doc->addCustomTag('<meta property="og:type" content="article"/>');		
		//$doc->addCustomTag('<meta property="og:title" content="'.trim($doc->getTitle()).'"/>');
		//$doc->addCustomTag( '<meta property="og:site_name" content="'.$app->getCfg( 'sitename' ).'"/>');
		//$doc->addCustomTag( '<meta property="og:description" content="'.$app->getCfg( 'MetaDesc' ).'"/>');
        }
        
        /*LINK SLIDER*/
        if ($gantry->get('slider-enabled') == '1' && (is_home() == 'true' || is_front_page())) {?>
            <script type="text/javascript">
                   var autoslide_time = <?php echo $gantry->get('slider-animspeed'); ?>;
            </script>
            <?php
            $gantry->addScript('slider_freshcubes.js');
            
        }
        /* LINK TWITTER */
        if ($gantry->get('post-twitter-enabled')) 
            $gantry->addScript('twitter.js');
        
        /* LINK GOOGLE PLUS */
        if ($gantry->get('post-gplus-enabled')){ 
            $gantry->addScript('googleplus.js');
        }
      
        ?>
    </head>
    <body <?php echo $gantry->displayBodyTag(); ?>>
        <?php if($gantry->get('background-image') == 'fix') { ?>            
            <img id="site-background-image" style="position: fixed; top:0; left:0; z-index: -1;" src="<?php echo get_template_directory_uri(); ?>/images/body/background.jpg">    
        <?php } ?>
        <?php if($gantry->get('background-patern') == 'yes') { ?>
        ...
        <div id="rt-header-wrapper">
            <div id="rt-logo-wrapper">
                <div id="rt-logo-content">
                    <?php if ($gantry->countModules('logo')) { ?>
                ...
                <div id="rt-navigation-content">
                    <div class="rt-container">
                        <div id="rt-top">
                            <div id="rt-top-left">
                                <div class="rt-top-a">
                                    <?php if ($gantry->countModules('top-a')) { ?>
                                    <?php echo $gantry->displayModules('top-a', 'basic', 'standard'); ?>
                                    <?php  } ?> 
                                    <div class="clear"></div>
                                </div>
                            </div>
                            <div id="rt-top-right">
                                <div class="rt-top-c">
                                    <?php if ($gantry->countModules('top-c')) { ?>
                                    <?php echo $gantry->displayModules('top-c', 'basic', 'standard'); ?>
                                    <?php  } ?> 
                                    <div class="clear"></div>
                                </div>
                                <div class="rt-top-b">
                                    <?php if ($gantry->countModules('top-b')) { ?>
                                    <?php echo $gantry->displayModules('top-b', 'basic', 'standard'); ?>
                                    <?php  } ?> 
                                    <div class="clear"></div>
                                </div>
                            </div>
                        ...
                        <div id="rt-menu">
                            <?php if ($gantry->countModules('navigation')) { ?>
        ...
        <div class="body-wrapper">
            ...
            <?php /** Begin Slider For Jaw Templates **/ if ($gantry->get('slider-enabled') == '1' && (is_home() == 'true' || is_front_page())) { ?>	 
                <div id="rt-featured"> 
                    <div class="rt-container">
					 <?php if ($gantry->get('slider-enabled') == '1' ) { ?>	 
                        <div id="rt-slider">
                            <?php include (TEMPLATEPATH . '/sliders/freshcubes.php'); ?>
                        </div>
						<?php } ?>
						 <?php if ($gantry->countModules('featured-right')) { ?>	 
                        <div id="rt-featured-right">
                            <?php echo $gantry->displayModules('featured-right', 'basic', 'standard'); ?>
                        </div>
						<?php } ?>
                        <div class="clear"></div>
                    </div>
                ...
                <?php
               /** END Slider For Jaw Templates **/ 
            } else { ?>      
                <?php /** Begin Slider For WP as widgets **/ 
                if (($gantry->countModules('featured-right') || $gantry->countModules('slider')) && (is_home() == 'true' || is_front_page())) : ?>
                <div id="rt-featured"> 
                    <div class="rt-container">
                       <?php if ($gantry->countModules('slider')) { ?>
                        <div id="rt-slider">
             ...
             <?php /** Slider For WP as widgets **/ endif; ?>
             <?php /** End Slider **/ } ?>
            
            <?php /** Begin Slider navigation For Jaw Templates **/ 
                if ($gantry->get('slidernav-enabled') == '1' && (is_home() == 'true' || is_front_page())) { ?>
            <div id="rt-slnav"> 
                    <div class="rt-container">
            <?php include (TEMPLATEPATH . '/sliders/freshcubes_nav.php'); ?>
                        <div class="clear"></div>
                    </div>
                </div>
            <?php /** End Slider navigation For Jaw Templates **/ } else { ?>
            
            <?php /** Begin Slider Navigation WP * */ if ($gantry->countModules('slnav') && (is_home() == 'true' || is_front_page()))  : ?>
                <div id="rt-slnav"> 
                    <div class="rt-container">
                        <?php echo $gantry->displayModules('slnav', 'basic', 'standard'); ?>
                        <div class="clear"></div>
                    </div>
                </div>
            <?php /** End Slider Navigation WP * */ endif; ?>
            ...
            <div id="rt-footer">
                <div class="rt-container">
                    <?php if ($gantry->countModules('footer-left-a') || $gantry->countModules('footer-left-b')) { ?>
                     
                    <div id="rt-footer-left">
                        <?php if ($gantry->countModules('footer-left-a')) { ?>
                        <div id="rt-footer-left-a">
                            <?php echo $gantry->displayModules('footer-left-a', 'basic', 'standard'); ?>
                        </div>
                        <?php } ?>
                        <?php if ($gantry->countModules('footer-left-b')) { ?>
                        <div id="rt-footer-left-b">
                            <?php echo $gantry->displayModules('footer-left-b', 'basic', 'standard'); ?>
                        </div>
                        <?php } ?>
                    </div>
                    <?php } ?>
                    <?php if ($gantry->countModules('footer-right-a')) { ?>
                    <div id="rt-footer-right">
                        <?php echo $gantry->displayModules('footer-right-a', 'basic', 'standard'); ?>
                    </div> 
                    <?php } ?>
                    <div class="clear"></div>
                </div>
        ...
        <?php /** End Debug * */ endif; ?>               
          <?php /** Begin Analytics * */ if ($gantry->countModules('panel')) : ?>
    ...
    <!-- aboutrui.com Baidu tongji analytics -->
<script type="text/javascript">
var _bdhmProtocol = (("https:" == document.location.protocol) ? " https://" : " http://");
document.write(unescape("%3Cscript src='" + _bdhmProtocol + "hm.baidu.com/h.js%3F01a0a422677a5600c1ba1a528a220267' type='text/javascript'%3E%3C/script%3E"));
</script>
</body>
</html>
<?php
