...
post_password = ''
ORDER BY comment_date_gmt DESC LIMIT 20";

$comments = $wpdb->get_results($sql);
$output = $pre_HTML;
$gravatar_status = ''; /* off if not using */

foreach ($comments as $comment) {

$f = md5(strtolower($comment->comment_author_email));
$a = get_bloginfo('wpurl') .'/wp-content/plugins/gravatar/'.$f.'.jpg';
$e = ABSPATH . 'wp-content/plugins/gravatar/'.$f.'.jpg';
$t = 1209600;
$d = get_bloginfo('wpurl'). '/wp-content/plugins/gravatar/default-gravatar.png';
if ( !is_file($e) || (time() - filemtime($e)) > $t ){
$r = get_option('avatar_rating');
$g = 'http://www.gravatar.com/avatar/'.$f.'?s=20&d='.$d.'&r='.$r;
copy($g, $e);
}
if (filesize($e) < 500) {copy($d, $e);}
?>

<?php if($gravatar_status == '') { ?>
<?php } ?>
<a href="<?php echo get_permalink($comment->ID); ?>#comment-<?php echo $comment->comment_ID; ?>" title="<?php echo $comment->post_title; ?>"><img class="topping" src="<?php echo $a ?>" alt="<?php echo $grav_namel ?>" />
</a>
