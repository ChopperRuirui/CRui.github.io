 ...
 */
function gantry_bp_ajax_querystring( $query_string, $object ) {
	global $bp;
	...
	return apply_filters( 'gantry_bp_ajax_querystring', $query_string, $object, $object_filter, $object_scope, $object_page, $object_search_terms, $object_extras );
}
add_filter( 'bp_ajax_querystring', 'gantry_bp_ajax_querystring', 10, 2 );

/* This function will simply load the template loop for the current object. On an AJAX request */
function gantry_bp_object_template_loader() {
	...
	gantry_bp_locate_type( array( "$object/$object-loop.php" ), true );
}
add_action( 'wp_ajax_members_filter', 'gantry_bp_object_template_loader' );
add_action( 'wp_ajax_groups_filter',  'gantry_bp_object_template_loader' );
add_action( 'wp_ajax_blogs_filter',   'gantry_bp_object_template_loader' );
add_action( 'wp_ajax_forums_filter',  'gantry_bp_object_template_loader' );

// This function will load the activity loop template when activity is requested via AJAX
function gantry_bp_activity_template_loader() {
	global $bp, $gantry;
	...
	ob_start();
	gantry_bp_locate_type( array( 'activity/activity-loop.php' ), true );
	$result['contents'] = ob_get_contents();
	$result['feed_url'] = apply_filters( 'gantry_bp_activity_feed_url', $feed_url, $scope );
	ob_end_clean();

	echo json_encode( $result );
}
add_action( 'wp_ajax_activity_widget_filter', 'gantry_bp_activity_template_loader' );
add_action( 'wp_ajax_activity_get_older_updates', 'gantry_bp_activity_template_loader' );

/* AJAX update posting */
function gantry_bp_post_update() {
	global $bp;
...
add_action( 'wp_ajax_post_update', 'gantry_bp_post_update' );

/* AJAX activity comment posting */
function gantry_bp_new_activity_comment() {
	global $bp;
	...
	$template = gantry_bp_locate_type( 'activity/comment.php', false, false );
...
add_action( 'wp_ajax_new_activity_comment', 'gantry_bp_new_activity_comment' );

/* AJAX delete an activity */
function gantry_bp_delete_activity() {
	global $bp;
...
add_action( 'wp_ajax_delete_activity', 'gantry_bp_delete_activity' );

/* AJAX delete an activity comment */
function gantry_bp_delete_activity_comment() {
	global $bp;
...
add_action( 'wp_ajax_delete_activity_comment', 'gantry_bp_delete_activity_comment' );

/* AJAX mark an activity as a favorite */
function gantry_bp_mark_activity_favorite() {
	global $bp;

	bp_activity_add_user_favorite( $_POST['id'] );
	_e( 'Remove Favorite', 'buddypress' );
}
add_action( 'wp_ajax_activity_mark_fav', 'gantry_bp_mark_activity_favorite' );

/* AJAX mark an activity as not a favorite */
function gantry_bp_unmark_activity_favorite() {
	global $bp;

	bp_activity_remove_user_favorite( $_POST['id'] );
	_e( 'Favorite', 'buddypress' );
}
add_action( 'wp_ajax_activity_mark_unfav', 'gantry_bp_unmark_activity_favorite' );
 ...
 * @since 1.5
 */
function gantry_bp_get_single_activity_content() {
	...
	do_action_ref_array( 'gantry_bp_get_single_activity_content', array( &$activity ) );
...
add_action( 'wp_ajax_get_single_activity_content', 'gantry_bp_get_single_activity_content' );

/* AJAX invite a friend to a group functionality */
function gantry_bp_ajax_invite_user() {
	global $bp;
...
add_action( 'wp_ajax_groups_invite_user', 'gantry_bp_ajax_invite_user' );

/* AJAX add/remove a user as a friend when clicking the button */
function gantry_bp_ajax_addremove_friend() {
	global $bp;
...
add_action( 'wp_ajax_addremove_friend', 'gantry_bp_ajax_addremove_friend' );

/* AJAX accept a user as a friend when clicking the "accept" button */
function gantry_bp_ajax_accept_friendship() {
...
add_action( 'wp_ajax_accept_friendship', 'gantry_bp_ajax_accept_friendship' );

/* AJAX reject a user as a friend when clicking the "reject" button */
function gantry_bp_ajax_reject_friendship() {
...
add_action( 'wp_ajax_reject_friendship', 'gantry_bp_ajax_reject_friendship' );

/* AJAX join or leave a group when clicking the "join/leave" button */
function gantry_bp_ajax_joinleave_group() {
	global $bp;
...
add_action( 'wp_ajax_joinleave_group', 'gantry_bp_ajax_joinleave_group' );

/* AJAX close and keep closed site wide notices from an admin in the sidebar */
function gantry_bp_ajax_close_notice() {
	global $userdata;
...
add_action( 'wp_ajax_messages_close_notice', 'gantry_bp_ajax_close_notice' );

/* AJAX send a private message reply to a thread */
function gantry_bp_ajax_messages_send_reply() {
	global $bp;
...
add_action( 'wp_ajax_messages_send_reply', 'gantry_bp_ajax_messages_send_reply' );

/* AJAX mark a private message as unread in your inbox */
function gantry_bp_ajax_message_markunread() {
	global $bp;
...
add_action( 'wp_ajax_messages_markunread', 'gantry_bp_ajax_message_markunread' );

/* AJAX mark a private message as read in your inbox */
function gantry_bp_ajax_message_markread() {
	global $bp;
...
add_action( 'wp_ajax_messages_markread', 'gantry_bp_ajax_message_markread' );

/* AJAX delete a private message or array of messages in your inbox */
function gantry_bp_ajax_messages_delete() {
	global $bp;
...
add_action( 'wp_ajax_messages_delete', 'gantry_bp_ajax_messages_delete' );

/**
 * gantry_bp_ajax_messages_autocomplete_results()
 *
 ...
 * @return none
 */
function gantry_bp_ajax_messages_autocomplete_results() {
	global $bp;
...
add_action( 'wp_ajax_messages_autocomplete_results', 'gantry_bp_ajax_messages_autocomplete_results' );
