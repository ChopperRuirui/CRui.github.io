...
<div id="gantry-overrides">
	<div class="overrides-inner">
		<?php
		$toggleStatus = (!count($override_catalog)) ? ' class="hidden"' : '';
		?>
		<div id="overrides-actions">
			<div id="overrides-first">
				...
				if( $override_id == 0 ) {
					...
					echo $override_catalog[$override_id];
			...
			<?php
				...
				foreach ($override_catalog as $o_id => $o_name) {
					$overridesList[] = '<div class="overrides-action"><a href="' . str_replace("&", "&amp;", add_query_arg(array('page' => 'slideshow', 'override_id' => $o_id))) . '">' . $o_name . '</a></div>';
				}
				if( $override_id !== 0 ) {
					echo '<div class="overrides-action"><a href="' . str_replace("&", "&amp;", add_query_arg(array('page' => 'slideshow', 'override_id' => null)) ) . '">Default Settings</a></div>';
				}
			?>
			<?php echo implode("\n", $overridesList); ?>
			</div>
		</div>
		<input type="hidden" name="override_id" id="override_id" value="<?php echo $override_id ?>" />
...
<div class="metabox-holder has-right-sidebar">
	<div class="inner-sidebar">
		<div class="postbox">
			<div class="hndle"><h3>Settings</h3></div>
			<div class="inside">
			<table class="form-table"><tbody>
				<tr valign="top">
					<th>Theme</th>
					<td>
						<select name="slideshow[theme]">
							<option value="default" <?php selected( $slideshow['theme'], 'default' ) ?>>Default</option>
							<option value="orman" <?php selected( $slideshow['theme'], 'orman' ) ?>>Orman</option>
							<option value="pascal" <?php selected( $slideshow['theme'], 'pascal' ) ?>>Pascal</option>
						</select>
					</td>
				</tr>
				<tr valign="top">
					<th>Width</th>
					<td><input type="text" name="slideshow[width]" size="3" value="<?php echo $slideshow['width'] ?>" /></td>
				</tr>
				<tr valign="top">
					<th>Height</th>
					<td><input type="text" name="slideshow[height]" size="3" value="<?php echo $slideshow['height'] ?>" /></td>
				</tr>
				<tr valign="top">
					<th>Transition</th>
					<td>
						<select name="slideshow[transition]">
							<?php foreach( $transitions as $id => $label ) : ?>
							<option value="<?php echo $id ?>" <?php selected( $id, $slideshow['transition'] ) ?>><?php echo $label ?></option>
							<?php endforeach; ?>
						</select>
					</td>
				</tr>
				<tr valign="top">
					<th>Slices</th>
					<td><input type="text" name="slideshow[slices]" size="3" value="<?php echo $slideshow['slices'] ?>" /></td>
				</tr>
				<tr valign="top">
					<th>Animation Speed</th>
					<td><input type="text" name="slideshow[animSpeed]" size="3" value="<?php echo $slideshow['animSpeed'] ?>" /></td>
				</tr>
				<tr valign="top">
					<th>Pause Time</th>
					<td><input type="text" name="slideshow[pauseTime]" size="3" value="<?php echo $slideshow['pauseTime'] ?>" /></td>
				</tr>
				<tr valign="top">
					<th>Pause on hover</th>
					<td>
						<select name="slideshow[pauseOnHover]">
							<option value="enable" <?php selected( 'enable', $slideshow['pauseOnHover'] ) ?>>Enable</option>
							<option value="disable" <?php selected( 'disable', $slideshow['pauseOnHover'] ) ?>>Disable</option>
						</select>
					</td>
				</tr>
				<tr valign="top">
					<th>Direction nav buttons</th>
					<td>
						<select name="slideshow[directionNav]">
							<option value="enable" <?php selected( 'enable', $slideshow['directionNav'] ) ?>>Enable</option>
							<option value="disable" <?php selected( 'disable', $slideshow['directionNav'] ) ?>>Disable</option>
						</select>
					</td>
				</tr>
				<tr valign="top">
					<th>Control nav</th>
					<td>
						<select name="slideshow[controlNav]">
							<option value="enable" <?php selected( 'enable', $slideshow['controlNav'] ) ?>>Enable</option>
							<option value="disable" <?php selected( 'disable', $slideshow['controlNav'] ) ?>>Disable</option>
						</select>
					</td>
				</tr>
				<tr valign="top">
					...
					<td>
						<select name="slideshow[keyboardNav]">
							<option value="enable" <?php selected( 'enable', $slideshow['keyboardNav'] ) ?>>Enable</option>
							<option value="disable" <?php selected( 'disable', $slideshow['keyboardNav'] ) ?>>Disable</option>
						</select>
					</td>
				</tr>
			</tbody></table>
	...
	<div class="post-body-content">
			<?php //echo '<pre dir="ltr">';print_r($slideshow);echo '</pre>'; ?>
			<?php if( $override_id !== 0 ) : ?>
			<div id="enable_override_wrap"><label><input type="checkbox" name="override" id="enable_override" value="1" <?php if( isset( $slideshow['overrides'][$override_id] ) ) echo 'checked="checked"'; ?> /> <?php _e('Enable override?', $this->textdomain ) ?></label></div>
			<div id="override_opac"></div>
			<?php endif; ?>
			<ul id="slides">
				<?php if( ! empty( $slideshow['slides']['image'] ) ) : for( $i = 0; $i < count( $slideshow['slides']['image'] ); $i++ ) : ?>
				<li>
					<div class="handle"></div>
					<div class="remove"></div>
					<div class="slide">
						<input type="text" size="32" name="slideshow[slides][title][]" value="<?php echo $slideshow['slides']['title'][$i] ?>" />
						<input type="text" size="46" name="slideshow[slides][image][]" value="<?php echo $slideshow['slides']['image'][$i] ?>" />
						<input type="button" class="button-secondary media-upload" value="Upload" />
					</div>
				</li>
				<?php endfor; endif; ?>
			</ul>
			<input type="button" class="button-secondary" value="Add New Slide" id="new-slide" />
