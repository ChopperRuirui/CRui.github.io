...
/*
Plugin Name:	Gantry Slideshow
Description:	Slideshow manager for Gantry framework.
...
class Gantry_Slideshow {

	private $textdomain;

	function Gantry_Slideshow() {
		add_action( 'admin_menu', array( &$this, 'admin' ) );
		add_action( 'widgets_init', array( &$this, 'widgets_init' ) );
		add_action( 'template_redirect', array( &$this, 'slider_queue' ) );
	}

	function admin() {
		$options_page = add_theme_page(
			__( 'Slideshow', $this->textdomain ), // Name of page
			__( 'Slideshow', $this->textdomain ), // Label in menu
			'edit_theme_options',                  // Capability required
			'slideshow',                       // Menu slug, used to uniquely identify the page
			array( &$this, 'options_page' )            // Function that renders the options page
		);
		add_action( "load-{$options_page}", array( &$this, 'actions' ) );
		add_action( "admin_print_styles-{$options_page}", array( &$this, 'admin_queue' ) );
	}

	function options_page() {
		$slideshow = $this->get_slides();
		include_once( dirname( __FILE__ ) . '/screens/options.php' );
	}

	function actions() {
		// whole options is stored as an array. Each override is stored in 'overrides' sub array.
		if( isset( $_POST['slideshow'] ) ) {
			$options = get_option( 'gantry_slideshow', array() );
			if( ! empty( $_GET['override_id'] ) ) {
				$override_id = $_GET['override_id'];
				if( $_POST['override'] == 1 ) {
					$options['overrides'][$override_id] = $_POST['slideshow'];
				} else {
					unset( $options['overrides'][$override_id] );
				}
			} else {
				// default override
				$options = wp_parse_args( $_POST['slideshow'], $options );
			}
			update_option( 'gantry_slideshow', $options );
		}
	}
	...
	function admin_queue() {
		...
		wp_enqueue_script( 'mootools', $gantry->gantryUrl . '/js/mootools.js' );
		wp_enqueue_script( 'gantry', plugins_url( 'js/gantry.js', __FILE__ ) );
		wp_enqueue_style( 'gantry', $gantry->gantryUrl . '/admin/widgets/gantry-widgets.css' );
		add_thickbox();
		wp_enqueue_script( 'gantry-slideshow', plugins_url( 'js/slideshow.js', __FILE__ ), array( 'jquery-ui-sortable', 'media-upload' ) );
		wp_enqueue_style( 'gantry-slideshow', plugins_url( 'css/admin.css', __FILE__ ) );
	}

	function slider_queue() {
		wp_enqueue_script( 'nivoslider', plugins_url( 'nivo-slider/jquery.nivo.slider.pack.js', __FILE__ ), array( 'jquery' ), '2.7.1' );
		wp_enqueue_style( 'nivoslider', plugins_url( 'nivo-slider/nivo-slider.css', __FILE__ ), array(), '2.7.1' );
		$options = $this->get_slides();
		$theme = ( isset( $options['theme'] ) ) ? $options['theme'] : 'default';
		wp_enqueue_style( "nivoslider-{$theme}", plugins_url( "nivo-slider/themes/{$theme}/{$theme}.css", __FILE__ ), array(), '2.7.1' );
	}

	public static function get_slides() {
		global $gantry;

		$slideshow = get_option( 'gantry_slideshow', array() );
		$slideshow = wp_parse_args( $slideshow, self::_defaults() );
		if( is_admin() ) {
			if( isset( $_GET['override_id'] ) ) {
				$override_id = $_GET['override_id'];
				if( isset( $slideshow['overrides'][$override_id] ) ) {
					$slideshow['slides'] = $slideshow['overrides'][$override_id]['slides'];
				}
			}
		} else {
			foreach( $gantry->_override_tree as $override ) {
				...
				if( isset( $slideshow['overrides'][$override_id] ) ) {
					// set active slides to the slides of the current override
					$slideshow['slides'] = $slideshow['overrides'][$override_id]['slides'];
				}
			}
		}
	...
	public static function display() {
		...
		if( ! empty( $slideshow['slides']['image'] ) ) {
			if( file_exists( TEMPLATEPATH . '/plugins/slideshow/default.php' ) )
				require_once( TEMPLATEPATH . '/plugins/slideshow/default.php' );
			else
				require_once( trailingslashit( dirname( __FILE__ ) ) . 'screens/display.php' );
		}
	}

	function widgets_init() {
		include_once( trailingslashit( dirname( __FILE__ ) ) . 'widget.php' );
		register_widget( 'Gantry_Widget_Slideshow' );
	}
